...
/// ``CrewScopedInteractor`` (internal use).
/// 
/// Conform this protocol to other RIBs' `Listener` protocols, to mark ``CrewScopedInteractor`` 
/// as able to receive and will handle events coming from its descendants. 
protocol CrewScopedInteractable: Interactable {
    var router  : CrewScopedRouting? { get set }
    var listener: CrewScopedListener? { get set }
}
...
/// to be manipulated by ``CrewScopedRouter``.
/// 
/// By conforming this RIB's descendant's `Router` to this, 
/// you declare that said `Router` will bridge out ny interactions 
/// coming from ``CrewScopedRouter`` that manipulates 
/// the view its controlling.
protocol CrewScopedViewControllable: ViewControllable {}


/// Manages the UI of `CrewScopedRIB` and relation with another RIB.
final class CrewScopedRouter: ViewableRouter<CrewScopedInteractable, CrewScopedViewControllable>, CrewScopedRouting {
    ...
    override init (
        interactor    : CrewScopedInteractable, 
        viewController: CrewScopedViewControllable
