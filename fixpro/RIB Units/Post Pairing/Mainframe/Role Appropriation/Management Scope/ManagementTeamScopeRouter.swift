...
/// ``ManagementTeamScopeInteractor`` (internal use).
/// 
/// Conform this protocol to other RIBs' `Listener` protocols, to mark ``ManagementTeamScopeInteractor`` 
/// as able to receive and will handle events coming from its descendants. 
protocol ManagementTeamScopeInteractable: Interactable {
    var router  : ManagementTeamScopeRouting? { get set }
    var listener: ManagementTeamScopeListener? { get set }
}
...
/// to be manipulated by ``ManagementTeamScopeRouter``.
/// 
/// By conforming this RIB's descendant's `Router` to this, 
/// you declare that said `Router` will bridge out ny interactions 
/// coming from ``ManagementTeamScopeRouter`` that manipulates 
/// the view its controlling.
protocol ManagementTeamScopeViewControllable: ViewControllable {}


/// Manages the UI of `ManagementTeamScopeRIB` and relation with another RIB.
final class ManagementTeamScopeRouter: ViewableRouter<ManagementTeamScopeInteractable, ManagementTeamScopeViewControllable>, ManagementTeamScopeRouting {
    ...
    override init (
        interactor    : ManagementTeamScopeInteractable, 
        viewController: ManagementTeamScopeViewControllable
