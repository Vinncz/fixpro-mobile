...
/// Contract adhered to by ``SupportiveDocumentPreviewInteractor``, listing the attributes and/or actions 
/// that ``SupportiveDocumentPreviewRouter`` is allowed to access or invoke.
/// 
/// Conform this `Interactable` protocol with this RIB's children's `Listener` protocols.
protocol SupportiveDocumentPreviewInteractable: Interactable {
    
    
    /// Reference to ``SupportiveDocumentPreviewRouter``.
    var router: SupportiveDocumentPreviewRouting? { get set }
    
    
    /// Reference to this RIB's parent's Interactor.
    var listener: SupportiveDocumentPreviewListener? { get set }
...
/// that ``SupportiveDocumentPreviewRouter`` is allowed to access or invoke.
protocol SupportiveDocumentPreviewViewControllable: ViewControllable {
...
/// The attachment point of `SupportiveDocumentPreviewRIB`.
final class SupportiveDocumentPreviewRouter: ViewableRouter<SupportiveDocumentPreviewInteractable, SupportiveDocumentPreviewViewControllable> {
    
    
    /// Constructs an instance of ``SupportiveDocumentPreviewRouter``.
    /// - Parameter interactor: The interactor for this RIB.
    /// - Parameter viewController: The view controller for this RIB.
    override init(interactor: SupportiveDocumentPreviewInteractable, viewController: SupportiveDocumentPreviewViewControllable) {
...
/// Conformance extension to the ``SupportiveDocumentPreviewRouting`` protocol.
/// Contains everything accessible or invokable by ``SupportiveDocumentPreviewInteractor``.
extension SupportiveDocumentPreviewRouter: SupportiveDocumentPreviewRouting {}
