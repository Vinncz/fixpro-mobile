...
/// that ``SupportiveDocumentPreviewViewController`` is allowed to access or invoke.
protocol SupportiveDocumentPreviewPresentableListener: AnyObject {}
 
 

/// The visible region of `SupportiveDocumentPreviewRIB`.
final class SupportiveDocumentPreviewViewController: UIViewController {
    
    
    /// Reference to ``SupportiveDocumentPreviewInteractor``.
    weak var presentableListener: SupportiveDocumentPreviewPresentableListener?
...
/// Conformance to the ``SupportiveDocumentPreviewViewControllable`` protocol.
/// Contains everything accessible or invokable by ``SupportiveDocumentPreviewRouter``.
extension SupportiveDocumentPreviewViewController: SupportiveDocumentPreviewViewControllable {
...
/// Conformance extension to the ``SupportiveDocumentPreviewPresentable`` protocol.
/// Contains everything accessible or invokable by ``SupportiveDocumentPreviewInteractor``.
extension SupportiveDocumentPreviewViewController: SupportiveDocumentPreviewPresentable {}
