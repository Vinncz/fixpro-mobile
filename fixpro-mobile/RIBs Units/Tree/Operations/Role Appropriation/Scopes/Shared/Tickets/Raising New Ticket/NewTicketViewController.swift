...
/// that ``NewTicketViewController`` is allowed to access or invoke.
protocol NewTicketPresentableListener: AnyObject {}
 
 

/// The visible region of `NewTicketRIB`.
final class NewTicketViewController: UIViewController {
    
    
    /// Reference to ``NewTicketInteractor``.
    weak var presentableListener: NewTicketPresentableListener?
    
    
    /// Reference to ``NewTicketSwiftUIViewModel``.
    weak var viewModel: NewTicketSwiftUIViewModel?
    
    
    /// The SwiftUI view that is displayed by this view controller.
    var hostingController: UIHostingController<NewTicketSwiftUIView>?
    ...
    /// Constructs an instance of ``NewTicketSwiftUIView``, wraps them into `UIHostingController`,
    ...
    func buildHostingController() {
        ...
        let swiftUIView = NewTicketSwiftUIView(viewModel: viewModel)
...
/// Conformance to the ``NewTicketViewControllable`` protocol.
/// Contains everything accessible or invokable by ``NewTicketRouter``.
extension NewTicketViewController: NewTicketViewControllable {
...
/// Conformance extension to the ``NewTicketPresentable`` protocol.
/// Contains everything accessible or invokable by ``NewTicketInteractor``.
extension NewTicketViewController: NewTicketPresentable {
    
    
    /// Binds the ``NewTicketSwiftUIViewModel`` to the view controller.
    func bind(viewModel: NewTicketSwiftUIViewModel) {
        self.viewModel = viewModel
    }
    
    
    /// Unbinds the ``NewTicketSwiftUIViewModel`` from the view controller.
