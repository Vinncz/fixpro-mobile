...
/// that ``SupportiveDocumentListViewController`` is allowed to access or invoke.
protocol SupportiveDocumentListPresentableListener: AnyObject {}
 
 

/// The visible region of `SupportiveDocumentListRIB`.
final class SupportiveDocumentListViewController: UIViewController {
    
    
    /// Reference to ``SupportiveDocumentListInteractor``.
    weak var presentableListener: SupportiveDocumentListPresentableListener?
    
    
    /// Reference to ``SupportiveDocumentListSwiftUIViewModel``.
    weak var viewModel: SupportiveDocumentListSwiftUIViewModel?
    
    
    /// The SwiftUI view that is displayed by this view controller.
    var hostingController: UIHostingController<SupportiveDocumentListSwiftUIView>?
    ...
    /// Constructs an instance of ``SupportiveDocumentListSwiftUIView``, wraps them into `UIHostingController`,
    ...
    func buildHostingController() {
        ...
        let swiftUIView = SupportiveDocumentListSwiftUIView(viewModel: viewModel)
...
/// Conformance to the ``SupportiveDocumentListViewControllable`` protocol.
/// Contains everything accessible or invokable by ``SupportiveDocumentListRouter``.
extension SupportiveDocumentListViewController: SupportiveDocumentListViewControllable {
...
/// Conformance extension to the ``SupportiveDocumentListPresentable`` protocol.
/// Contains everything accessible or invokable by ``SupportiveDocumentListInteractor``.
extension SupportiveDocumentListViewController: SupportiveDocumentListPresentable {
    
    
    /// Binds the ``SupportiveDocumentListSwiftUIViewModel`` to the view controller.
    func bind(viewModel: SupportiveDocumentListSwiftUIViewModel) {
        self.viewModel = viewModel
    }
    
    
    /// Unbinds the ``SupportiveDocumentListSwiftUIViewModel`` from the view controller.
