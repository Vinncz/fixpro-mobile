...
/// that ``MemberDetailRouter`` is allowed to access or invoke.
/// 
/// Conform this `Interactable` protocol with this RIB's children's `Listener` protocols.
protocol MemberDetailInteractable: Interactable {
    
    
    /// Reference to ``MemberDetailRouter``.
    var router: MemberDetailRouting? { get set }
    
    
    /// Reference to this RIB's parent's Interactor.
    var listener: MemberDetailListener? { get set }
...
/// that ``MemberDetailRouter`` is allowed to access or invoke.
protocol MemberDetailViewControllable: ViewControllable {
...
/// The attachment point of `MemberDetailRIB`.
final class MemberDetailRouter: ViewableRouter<MemberDetailInteractable, MemberDetailViewControllable> {
    
    
    /// Constructs an instance of ``MemberDetailRouter``.
    /// - Parameter interactor: The interactor for this RIB.
    /// - Parameter viewController: The view controller for this RIB.
    override init(interactor: MemberDetailInteractable, viewController: MemberDetailViewControllable) {
...
/// Conformance extension to the ``MemberDetailRouting`` protocol.
/// Contains everything accessible or invokable by ``MemberDetailInteractor``.
extension MemberDetailRouter: MemberDetailRouting {
