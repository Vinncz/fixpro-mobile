...
/// Contract adhered to by ``CrewInvitingRouter``, listing the attributes and/or actions 
/// that ``CrewInvitingInteractor`` is allowed to access or invoke.
protocol CrewInvitingRouting: ViewableRouting {
...
/// that ``CrewInvitingInteractor`` is allowed to access or invoke.
protocol CrewInvitingPresentable: Presentable {
    
    
    /// Reference to ``CrewInvitingInteractor``.
    var presentableListener: CrewInvitingPresentableListener? { get set }
...
/// Contract adhered to by the Interactor of `CrewInvitingRIB`'s parent, listing the attributes and/or actions
/// that ``CrewInvitingInteractor`` is allowed to access or invoke.
protocol CrewInvitingListener: AnyObject {
    func dismissCrewInviting(didInvite: Bool)
}



/// The functionality centre of `CrewInvitingRIB`, where flow, communication, and coordination
/// are determined and initiated from.
final class CrewInvitingInteractor: PresentableInteractor<CrewInvitingPresentable>, CrewInvitingInteractable {
    
    
    /// Reference to ``CrewInvitingRouter``.
    weak var router: CrewInvitingRouting?
    
    
    /// Reference to this RIB's parent's Interactor.
    weak var listener: CrewInvitingListener?
    
    
    /// Reference to the component of this RIB.
    var component: CrewInvitingComponent
    
    
    /// Bridge to the ``CrewInvitingSwiftUIVIew``.
    private var viewModel: CrewInvitingSwiftUIViewModel
    ...
    /// Constructs an instance of ``CrewInvitingInteractor``.
    /// - Parameter component: The component of this RIB.
    init(component: CrewInvitingComponent, presenter: CrewInvitingPresentable, ticket: FPTicketDetail) {
        self.component = component
        self.ticket = ticket
        self.viewModel = CrewInvitingSwiftUIViewModel(ticket: ticket, authContext: component.authorizationContext)
    ...
    private func configureViewModel() {
        ...
        viewModel.didIntendToCancel = { [weak self] in 
            guard let self else { return }
            listener?.dismissCrewInviting(didInvite: false)
        }
        viewModel.didIntendToInvite = { [weak self] in 
            guard let self else { return }
            if try await self.submitDelegation() {
                listener?.dismissCrewInviting(didInvite: true)
            }
        }
        Task { [weak self] in
            guard let self else { return }
...
extension CrewInvitingInteractor {
...
//                        )
    ...
    func submitDelegation() async throws -> Bool {
        async let request = component.networkingClient.gateway.postTicketHandlers(.init(
            ...
            headers: .init(accept: [.init(contentType: .json)]),
            body: .json(.init(
                data: viewModel.crewInvitingDetails.map { detail in 
...
/// Conformance to the ``CrewInvitingPresentableListener`` protocol.
/// Contains everything accessible or invokable by ``CrewInvitingViewController``.
extension CrewInvitingInteractor: CrewInvitingPresentableListener {}
